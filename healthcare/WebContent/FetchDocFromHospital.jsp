...
<%@ page import="healthcare.bean.User" %>
    ...
    PreparedStatement stmt = conn.prepareStatement("select p3.hname,p3.dname,a.aid,a.date,a.session,a.day,a.pid from (select p1.hname hname,p1.haid aid ,p2.dname from (select h.name as hname,a.aid as haid from appointments a join hospital h on a.id=h.id) p1 join (select d.name as dname,a1.aid as daid from appointments a1 join doctor d on a1.mcid=d.mcid) p2 on p1.haid=p2.daid) p3,appointments a where p3.aid=a.aid and a.id=? and a.mcid=? and a.status is not null");
	...
	while(rs.next())
	{
		out.println("<tr><td>"+rs.getString(3)+"</td><td>"+rs.getString(7)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5)+"</td><td>"+rs.getString(6)+"</td></tr>");
	}
	out.println("</table>");
